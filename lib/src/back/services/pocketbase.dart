...
class PocketBaseConnector {
  ...
  Future<void> signInWithGoogle(context) async {
    if (alreadyClick == 0) {
      ...
      try {
        ...
        if (_pocketBase.authStore.isValid) {
          if (DateTime.parse(authData2['record']['created']) >=
        ...
        var path = await ImageDownloader.findPath(imageId);*/
              ...
              await _pocketBase
                  .collection('users')
                  .update(authData2['record']['id'], body: body);
  ...
  Future<void> deleteUser(String recordId) {
  ...
  Future<User?> findUser(String email) async {
    ...
    return _pocketBase
        ...
        .then((value) {
          debugPrint(
            'User found with email $email and data retrieved and parsed.',
          );
  ...
  Future<List<RecordModel>> getCollectionFullListOrderExpanded(
    ...
    return _pocketBase
        .collection(collectionId)
        .getFullList(sort: order, expand: expand);
  }
  ...
  Future<List<RecordModel>> getCollectionDataWithFilterExpand(
    ...
    return _pocketBase
        .collection(collectionId)
        .getList(page: 1, perPage: 500, filter: query, expand: expand)
        .then((value) => value.items);
  }
  ...
  Future<List<RecordModel>> getCollectionFullDataWithFilterExpand(
    ...
    return _pocketBase
        .collection(collectionId)
        .getFullList(filter: query, expand: expand)
        .then((value) => value);
  }
  ...
  Future<int> getNewRegisterLast24h() {
    return http
        .get(Uri.parse('https://api.mymangatheque.com/newRegisterLast24h'))
        .then((response) {
  ...
  Future<String> getNewRegisterLastMonth() {
    return http
        .get(Uri.parse('https://api.mymangatheque.com/newRegisterLastMonth'))
        .then((response) {
          if (response.statusCode == 200) {
  ...
  Future<int> getNumberOwnedManga(String id) async {
    ...
    try {
      ...
      final result = await _pocketBase
          .collection('owned')
  ...
  Future<void> removeVolumeFromOwned(String userId, String volumeId) async {
    final result = await _pocketBase
        .collection('owned')
        .getFullList(filter: "user='$userId'&&volume='$volumeId'");
  ...
  Future<void> changeReadState(
    ...
    final result = await _pocketBase
        ...
        .getFullList(filter: "user='$userId'&&volume='$volumeId'");
    final id = json.decode(result.toString())[0]['id'];
    final body = <String, dynamic>{"readed": readState};
  ...
  Future<void> addSubSeriesToFollowed(String userId, String subSeriesId) async {
    final body = <String, dynamic>{"user": userId, "sub_serie": subSeriesId};
  ...
  Future<void> removeSubSeriesToFollowed(
    ...
    final result = await _pocketBase
        .collection('followed')
        .getFullList(filter: "user='$userId'&&sub_serie='$subSeriesId'");
    final id = json.decode(result.toString())[0]['id'];
