...
class _SeriePageState extends State<SeriePage> {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<List<RecordModel>>(
      future: connector.getOneExpand('series', widget.serieId, 'subSeries.volumes,subSeries.editors,genres,authors'),
      builder: (BuildContext context, snapshot) {
        ...
        if (snapshot.hasData && snapshot.data != null) {
          ...
          final List<Map<String, dynamic>> subSeries = (rawSubSeries is List)
              ? rawSubSeries.map<Map<String, dynamic>>((e) => (e is Map) ? Map<String, dynamic>.from(e) : <String, dynamic>{}).toList()
              : <Map<String, dynamic>>[];
          ...
          return Scaffold(
            appBar: AppBar(
              title: Text(title, style: const TextStyle(fontSize: 25, fontWeight: FontWeight.w300)),
                    ...
                    children: [
                      ...
                      Padding(
                        ...
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('${localizations.genres} :', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                            SingleChildScrollView(
                              scrollDirection: Axis.horizontal,
                              child: Padding(
                                padding: const EdgeInsets.symmetric(vertical: 5),
                                child: Row(children: [for (var i = 0; i < genres.length; i += 1) MyGenresShow(data: genres[i])]),
                              ),
                            ),
                          ],
                        ),
                      ...
                      Padding(
                        ...
                        child: (subSeries.isEmpty)
                            ? const SizedBox()
                            : (subSeries.length == 1)
                            ? Text("${localizations.subSerie} :", style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold))
                            : Text("${localizations.subSeries} :", style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                      ),
                      for (var i = 0; i < subSeries.length; i += 1)
                        Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            MySubSeriesTile(data: subSeries[i], initRoute: widget.initRoute),
                            if (i != subSeries.length - 1) MyLine(width: MediaQuery.of(context).size.width, vertical: 10, horizontal: 0),
                          ],
                        ),
                      MyLine(width: MediaQuery.of(context).size.width, vertical: 10.0, horizontal: 0.0),
                      (authors.isEmpty)
                          ? const SizedBox()
                          : (authors.length == 1)
                          ? Padding(
