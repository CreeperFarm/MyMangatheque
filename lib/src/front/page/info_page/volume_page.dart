...
class _VolumePageState extends ConsumerState<VolumePage> {
  ...
  static const String _volumeExpandFields =
      'subSeries.authors,subSeries.editors';
    ...
    if (userId != null) {
      ...
      try {
        ...
        RuntimeLocalization.debug(
          en: 'Unable to resolve the volume owned state: $e',
          fr: 'Impossible de déterminer l’état de possession du volume : $e',
        );
      }
      if (!mounted) return;
      setState(() {
      ...
      if (owned) {
        ...
        try {
          ...
          RuntimeLocalization.debug(
            en: 'Unable to resolve the volume read state: $e',
            fr: 'Impossible de déterminer l’état de lecture du volume : $e',
          );
        }
        if (!mounted) return;
        setState(() {
    ...
    if (userId != null) {
      try {
        ...
        final data = result.isNotEmpty
            ? Map<String, dynamic>.from(result.first.data)
            : <String, dynamic>{};
        final expand = _asMap(data['expand']);
        final subSeries = _pickFirstMap(
          expand['sub_series'],
          expand['subseries'],
        );
        ...
        RuntimeLocalization.debug(
          en: 'Unable to resolve the sub-series follow state: $e',
          fr: 'Impossible de déterminer l’état de suivi de la sous-série : $e',
        );
        if (!mounted) return;
        setState(() {
          isSubSeriesFollowed = false;
  ...
  Future<void> _handleReadPressed(String volumeId) async {
    ...
    final nextReadState = !isVolumeReaded;
    try {
      await connector.changeReadState(userId, volumeId, nextReadState);
      await _refreshOwnedLibrary();
      if (!mounted) return;
      setState(() {
        isVolumeReaded = nextReadState;
      });
    } catch (error) {
      if (!mounted) return;
      _showMutationError(error);
  ...
  Future<void> _openReviewQuestionnaire(String volumeId) async {
    ...
    final favCharactersController = TextEditingController(
      text:
          ((existing?['favCharacters'] as List<dynamic>?) ?? const <dynamic>[])
                  ...
                  children: [
                    ...
                    TextField(
                      ...
                      decoration: InputDecoration(
                        labelText: localizations.favoriteCharacters,
                        hintText: localizations.favoriteCharactersHint,
                      ),
                    ),
                    const SizedBox(height: 8),
                    TextField(
                      ...
                      decoration: InputDecoration(
                        ...
                        hintText: localizations.reviewCommentHint,
                      ),
                    ),
                  ],
                ),
                      ...
                      : () async {
                          ...
                          try {
                            ...
                            ScaffoldMessenger.maybeOf(context)?.showSnackBar(
                              SnackBar(
                                content: Text(
                                  localizations.reviewSubmittedSuccess,
                                ),
                              ),
                            );
                          } catch (e) {
                            ...
                            ScaffoldMessenger.maybeOf(context)?.showSnackBar(
                              SnackBar(
                                content: Text(
                                  localizations.reviewSubmitFailed('$e'),
                                ),
                              ),
                            );
                          }
  ...
  Map<String, String> _normalizeBookLink(dynamic raw) {
    ...
    return <String, String>{
      'url': url,
      'seller': host.isEmpty ? '' : host.replaceFirst('www.', ''),
      'available': '',
    };
  }
    ...
    return FutureBuilder<List<RecordModel>>(
      ...
      builder: (BuildContext context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          RuntimeLocalization.debug(
            en: 'Unable to load the volume page: ${snapshot.error}',
            fr: 'Impossible de charger la page du volume : ${snapshot.error}',
          );
          return Scaffold(
          ...
          final expand = _asMap(data['expand']);
          final contain =
              (data['contain'] as List<dynamic>?) ?? const <dynamic>[];
          final containsExpanded = _asMapList(expand['contains']);
          final authors = expand['authors'] is List
              ? expand['authors'] as List<dynamic>
              : _asMapList(expand['authors']);
          final subSeries = _pickFirstMap(
            expand['sub_series'],
            expand['subseries'],
          );
          final subSeriesExpand = _asMap(subSeries['expand']);
          final expandedEditor = _pickFirstMap(
            expand['editor'],
            expand['editors'],
          );
          final nestedEditor = _pickFirstMap(
            subSeriesExpand['editor'],
            subSeriesExpand['editors'],
          );
          final editor = expandedEditor.isNotEmpty
              ? expandedEditor
              : nestedEditor;
          final series =
              _pickFirstMap(
          ...
          final bookLinksRaw =
              (data['book_link'] as List<dynamic>?) ?? const <dynamic>[];
          final bookLinks = bookLinksRaw
              .map(_normalizeBookLink)
              .where((entry) => entry['url']?.isNotEmpty == true)
              .toList();
          final isAdultVolume = _toBool(data['over18']);
          final shouldBlurAdultCover = isAdultVolume && !_adultContentEnabled;
          final priceValue =
              num.tryParse(data['price']?.toString() ?? '') ?? -1;
          final eanValue = data['ean']?.toString().trim() ?? '';
          final pageNumber = data['info'] is Map<String, dynamic>
              ? (data['info'] as Map<String, dynamic>)['pageNumber']
              : null;
                            ...
                            child: Container(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text(
                                    localizations
                                        .adultContentBlockedDescription,
                                    textAlign: TextAlign.center,
                                  ),
                                  ...
                                  ElevatedButton(
                                    ...
                                    child: Text(
                                      localizations.openContentSettings,
                                    ),
                                  ),
                                ],
                              ),
                  ...
                  child: Column(
                    ...
                    children: [
                                  ...
                                  child: Container(
                                    ...
                                    child: Padding(
                                      ...
                                      child: ElevatedButton.icon(
                                        ...
                                        onPressed: _isUpdatingRead
                                            ? null
                                            : () =>
                                                  _handleReadPressed(volumeId),
                                        label: Text(
                                          (!isVolumeReaded)
                                              ? localizations.read
                          ...
                          ? Padding(
                              ...
                              child: Container(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    Text(
                                      localizations.reviewQuestionnaireTitle,
                                      style: TextStyle(
                                        fontSize: 16,
                                        fontWeight: FontWeight.bold,
                                      ),
                                    ...
                                    Text(
                                      localizations
                                          .reviewQuestionnaireDescription,
                                    ),
                                    ...
                                    Align(
                                      ...
                                      child: ElevatedButton.icon(
                                        onPressed: () =>
                                            _openReviewQuestionnaire(volumeId),
                                        icon: const Icon(Icons.rate_review),
                                        label: Text(
                                          localizations.openQuestionnaire,
                                        ),
                                      ),
                                    ),
                                  ],
                            ...
                            (eanValue.isEmpty || eanValue == '0')
                                ...
                                : Padding(
                                    ...
                                    child: MyIconTextLabel(
                                      ...
                                      text: '${localizations.ean} : $eanValue',
                                      heightIcon: 30,
                                    ),
                                  ),
                            (pageNumber == null)
                                ...
                                : Padding(
                                    ...
                                    child: MyIconTextLabel(
                                      ...
                                      text:
                                          "${localizations.numberOfPages} : ${pageNumber.toString()}",
                                      heightIcon: 30,
                                    ),
                                  ),
                      ...
                      FutureBuilder<List<Map<String, dynamic>>>(
                        ...
                        future: connector.getVolumeReviews(
                          volumeId,
                          includePendingForUserId: _connectedUserId(),
                        ),
                        builder: (context, reviewsSnapshot) {
                          ...
                          if (reviews.isEmpty) {
                            return Padding(
                              ...
                              child: Text(
                                localizations.noReviewsForThisVolumeYet,
                              ),
                            );
                          }
